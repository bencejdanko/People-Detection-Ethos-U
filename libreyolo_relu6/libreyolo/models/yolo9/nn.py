...
def create_activation(activation=True):
    ...
    Default is ReLU6 (replaces original SiLU) for NPU/Ethos-U55 compatibility:
    - No exponential op (unlike SiLU/Swish) — maps to integer arithmetic on NPU
    - Clips at 6, improving INT8 quantization accuracy vs unbounded ReLU
    - Fully supported by TFLite and onnx2tf without unsupported-op CPU fallback
    """
